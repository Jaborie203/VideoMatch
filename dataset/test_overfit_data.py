...
class DAVIS_OVER_FIT_TEST(Dataset):
    ...
    def __init__(self, train=True,
        ...
        images = np.sort(os.listdir(os.path.join(db_root_dir, 'JPEGImages/480p/bear')))
        img_list = list(map(lambda x: os.path.join('JPEGImages/480p/bear', x), images))
        lab = np.sort(os.listdir(os.path.join(db_root_dir, 'Annotations/480p/bear')))
        labels = list(map(lambda x: os.path.join('Annotations/480p/bear', x), lab))
        ...
        print('Done initializing ' + 'bear' + ' Dataset')
...
if __name__ == '__main__':
    ...
    dataset = DAVIS_OVER_FIT_TEST(db_root_dir='../../../DAVIS-2016',
