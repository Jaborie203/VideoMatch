...
class DAVIS_OVER_FIT_TEST1(Dataset):
    ...
    def __init__(self, train=True,
        ...
        print('Done initializing ' + file_name + ' Dataset')
    ...
    def __getitem__(self, idx):
        if self.train:
            img_1, img_t, gt_1, gt_t = self.make_img_gt_pair_train()
        else:
    ...
    def make_img_gt_pair_train(self):
        """
        Make the image-ground-truth pair
        """
        idx_list = np.random.choice(self.img_num, 2, replace=False)
        img_1 = cv2.imread(os.path.join(self.db_root_dir, self.img_list[idx_list[0]]))
        img_t = cv2.imread(os.path.join(self.db_root_dir, self.img_list[idx_list[1]]))
        label_1 = cv2.imread(os.path.join(self.db_root_dir, self.labels[idx_list[0]]), 0)
        label_t = cv2.imread(os.path.join(self.db_root_dir, self.labels[idx_list[1]]), 0)
        #img->array
